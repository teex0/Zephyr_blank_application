...
MEMORY
    ...
    SRAM0 ( rw ) : ORIGIN = (536870912), LENGTH = (786432) PSRAM ( rw ) : ORIGIN = (2415919104), LENGTH = (8388608) EXTMEM ( rw ) : ORIGIN = (1879048192), LENGTH = (67108864)
...
/DISCARD/ :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    SRAM0 (NOLOAD) : { __SRAM0_start = .; KEEP(*(SRAM0)) KEEP(*(SRAM0.*)) __SRAM0_end = .; } > SRAM0 __SRAM0_size = __SRAM0_end - __SRAM0_start; __SRAM0_load_start = LOADADDR(SRAM0); PSRAM (NOLOAD) : { __PSRAM_start = .; KEEP(*(PSRAM)) KEEP(*(PSRAM.*)) __PSRAM_end = .; } > PSRAM __PSRAM_size = __PSRAM_end - __PSRAM_start; __PSRAM_load_start = LOADADDR(PSRAM); EXTMEM (NOLOAD) : { __EXTMEM_start = .; KEEP(*(EXTMEM)) KEEP(*(EXTMEM.*)) __EXTMEM_end = .; } > EXTMEM __EXTMEM_size = __EXTMEM_end - __EXTMEM_start; __EXTMEM_load_start = LOADADDR(EXTMEM);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
